...
export default function CatSprite({ charac_id }) {
  ...
  useEffect(() => {
    elmnt = document.getElementById(charac_id);
    if (elmnt) {
      elmnt.style.position = "absolute";
      elmnt.style.left = `${position.x}px`;
      elmnt.style.top = `${position.y}px`;
    }
  }, [charac_id, position]);
  ...
  const dragMouseDown = (e) => {
    ...
    document.onmousemove = (e) => {
      ...
      const nextX = position.x - startX + e.clientX;
      const nextY = position.y - startY + e.clientY;
  ...
  const closeDragElement = () => {
    ...
    if (elmnt) {
      // Check if the sprite is outside the PreviewArea boundaries
      const previewArea = document.getElementById("previewArea"); // Change this to the actual ID of your PreviewArea component
      if (previewArea) {
        const spriteRect = elmnt.getBoundingClientRect();
        const previewAreaRect = previewArea.getBoundingClientRect();
  
        if (
          spriteRect.right < previewAreaRect.left ||
          spriteRect.left > previewAreaRect.right ||
          spriteRect.bottom < previewAreaRect.top ||
          spriteRect.top > previewAreaRect.bottom
        ) {
          // The sprite is outside the PreviewArea, reset its position
